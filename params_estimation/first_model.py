...
class ParameterEstimation:
    ...
    def __init__(self, actual, stimulus, raf, mek, erk, r1, r2, r3, r_1, r_2, r_3, time, num_iter, learning_rate=1e-3, threshold=1e-6):
    ...
    def simulate_concentration(self):
        ...
        for i in range(1, steps):
            ...
            d_stimulus = -self.r0 * stimulus[i-1]
            d_raf = self.r0 * stimulus[i-1] + self.r_3 * erk[i-1] + self.r_1 * mek[i-1] - self.r3 * raf[i-1] - self.r1 * raf[i-1]
            d_mek = self.r1 * raf[i-1] + self.r_2 * erk[i-1] - self.r_1 * mek[i-1] - self.r2 * mek[i-1]
            d_erk = self.r3 * raf[i-1] + self.r2 * mek[i-1] - self.r_2 * erk[i-1] - self.r_3 * erk[i-1] - self.d * erk[i-1]
    ...
    def gradient_descent(self, r0, d):
        ...
        return self.r0, self.d, self.mse, self.populations
...
ngf_model = ParameterEstimation(actual_ngf, ngf, raf, mek, erk, nr1, nr2, nr3, nr_1, nr_2, nr_3, time, num_iter)
egf_model = ParameterEstimation(actual_egf, egf, raf, mek, erk, er1, er2, er3, er_1, er_2, er_3, time, num_iter)
